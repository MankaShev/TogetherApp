...
class PersonalCollectionsAdapter(
    ...
    private val onItemClick: (Collection) -> Unit  // Для обычных подборок
) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    private var items = listOf<ListItem>()

    fun submitList(collections: List<Collection>) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return when (viewType) {
            0 -> {
                // Для кнопки "Создать" используем тот же layout, но заполняем по-другому
    ...
    class CollectionViewHolder(
        ...
        fun bind(collection: Collection) {
            ...
            //placesView.text = "${collection.placeCount} мест"
